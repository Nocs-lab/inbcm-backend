...
exports.gerarPDFRecibo = gerarPDFRecibo;
 ...
 */
async function buscaDeclaracao(declaracaoId) {
    const declaracao = await models_1.Declaracoes.findById(declaracaoId);
    if (!declaracao) {
 ...
 */
async function buscaMuseu(museuId) {
    const museu = await models_1.Museu.findById(museuId);
    if (!museu) {
        throw new Error(`Museu não encontrado para o ID especificado: ${museuId}`);
    }
    return museu;
}
async function buscaUsuario(usuarioId) {
    const usuario = await models_1.Usuario.findById(usuarioId);
    if (!usuario) {
 ...
 */
function formatarDadosRecibo(declaracao, museu, usuario) {
    const totalBensDeclarados = (declaracao.arquivistico.quantidadeItens || 0) +
 ...
 */
async function gerarPDFRecibo(declaracaoId) {
    const fonts = {
        Roboto: {
            normal: path_1.default.resolve("fonts/Roboto-Regular.ttf"),
            bold: path_1.default.resolve("fonts/Roboto-Medium.ttf"),
            italics: path_1.default.resolve("fonts/Roboto-Italic.ttf"),
            bolditalics: path_1.default.resolve("fonts/Roboto-MediumItalic.ttf")
        }
    };
