...
export class ImportacaoController {
  static async iniciarImportacao(req: Request, res: Response) {
    ...
    if (
      emAndamento &&
      emAndamento.usuario &&
      typeof emAndamento.usuario === "object"
    ) {
      const usuario = emAndamento.usuario as any

      const iniciadoEmFormatado = new Date(
        emAndamento.iniciadoEm
    ...
    res
      .status(202)
      .json({ message: "Importação iniciada", importacaoId: importacao._id })

    setImmediate(async () => {
      if (mongoose.connection.readyState !== 1) {
        logger.warn(
          "Banco de dados ainda não está conectado. Aguardando para iniciar a importação."
        )
        return
      }
      ...
      try {
        ...
        const totalConcluidas = await Importacao.countDocuments({
          status: "concluida"
        })
  ...
  static async getStatusImportacao(req: Request, res: Response) {
    ...
    let mensagem = ""
    const dataHoraFormatada = new Date(importacao.iniciadoEm).toLocaleString(
      "pt-BR",
      {
        timeZone: "America/Sao_Paulo",
        dateStyle: "short",
    ...
    switch (importacao.status) {
      case "em_andamento":
        ...
        break
      case "concluida":
        mensagem = `Importação concluída em ${new Date(
          importacao.finalizadoEm!
        ).toLocaleString("pt-BR", {
          timeZone: "America/Sao_Paulo",
          dateStyle: "short",
          timeStyle: "short"
