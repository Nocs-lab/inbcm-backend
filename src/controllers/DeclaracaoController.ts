...
class DeclaracaoController {
  ...
  async criarDeclaracao(req: Request, res: Response) {
    try {
      ...
      let declaracaoExistente:
        | (mongoose.Document<unknown, {}, DeclaracaoModel> &
            DeclaracaoModel &
      ...
      if (idDeclaracao) {
        ...
        novaDeclaracaoData = {
          ...
          retificacaoRef: declaracaoExistente
            ? (declaracaoExistente._id as mongoose.Types.ObjectId)
            : undefined,
  ...
  async listarArquivistico(req: Request, res: Response) {
    ...
    try {
      ...
      const result = await this.declaracaoService.buscarItensArquivistico(
        museuId,
        ano
      )
      if (!result) {
        return res
          .status(404)
          .json({ message: "Itens arquivísticos não encontrados" })
      }
      ...
      if (error instanceof Error) {
        res.status(500).json({
          message: "Erro ao listar itens arquivísticos",
          error: error.message
        })
      } else {
        res
          .status(500)
          .json({ message: "Erro desconhecido ao listar itens arquivísticos" })
      }
    }
  }
  async listarBibliografico(req: Request, res: Response) {
    ...
    try {
      ...
      const result = await this.declaracaoService.buscarItensBibliograficos(
        museuId,
        ano
      )
      if (!result) {
  ...
  async listarMuseologico(req: Request, res: Response) {
    const { museuId, ano } = req.params
    try {
      const result = await this.declaracaoService.buscarItensMuseologicos(
        museuId,
        ano
      )
      if (!result) {
        return res
          .status(404)
          .json({ message: "Itens museológicos não encontrados" })
      }
      res.status(200).json(result)
    } catch (error) {
      ...
      if (error instanceof Error) {
        res.status(500).json({
          message: "Erro ao listar itens museológicos",
          error: error.message
        })
      } else {
        res
          .status(500)
          .json({ message: "Erro desconhecido ao listar itens museológicos" })
      }
    }
  }
}
