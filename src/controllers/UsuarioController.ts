...
class UsuarioController {
  ...
  async getUsersByProfile(req: Request, res: Response) {
    ...
    try {
      const usuarios = await Usuario.find({ profile: profileId, ativo: true }).populate('profile');
      if (usuarios.length === 0) {
        return res.status(404).json({ mensagem: "Nenhum usuário encontrado para este perfil." });
      }
      return res.status(200).json(usuarios);
    } catch (error) {
      console.error("Erro ao listar usuários por perfil:", error);
      return res.status(500).json({ mensagem: "Erro ao listar usuários por perfil." });
    }
  }
