...
class DeclaracaoService {
  ...
  async updateDeclaracao(
    ...
    try {
      ...
      } = await import("inbcm-xlsx-validator")
      const { museologico, bibliografico, arquivistico } = await import(
        "inbcm-xlsx-validator/schema.js"
      )
      ...
      if (arquivo) {
        ...
        let Modelo:
          | typeof Arquivistico
          | typeof Bibliografico
          | typeof Museologico
        let validate:
          | typeof validate_arquivistico
          | typeof validate_bibliografico
          | typeof validate_museologico
        switch (tipo) {
          case "arquivistico":
            Modelo = Arquivistico
        ...
        for (const item of arquivoData) {
          item.declaracao_ref = novaDeclaracao._id as string // Atualiza a referência para a nova declaração
          item.versao = novaVersao.toString() // Atribui a nova versão ao item
        }
  ...
  async buscarItensArquivistico(museuId: string, ano: string) {
    try {
      ...
      console.log(
        "Itens arquivisticos com maior versão encontrados após a consulta:",
        result
      )
      ...
      console.error(
        "Erro ao buscar itens arquivisticos com maior versão:",
        error
      )
      throw error
    }
  ...
  async buscarItensBibliograficos(museuId: string, ano: string) {
    try {
      ...
      console.log(
        "Itens bibliograficos com maior versão encontrados após a consulta:",
        result
      )
      ...
      console.error(
        "Erro ao buscar itens bibliograficos com maior versão:",
        error
      )
      throw error
    }
  ...
  async buscarItensMuseologicos(museuId: string, ano: string) {
    try {
      ...
      console.log(
        "Itens museológicos com maior versão encontrados após a consulta:",
        result
      )
      ...
      console.error(
        "Erro ao buscar itens museológicos com maior versão:",
        error
      )
      throw error
    }
