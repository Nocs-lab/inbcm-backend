...
const uploadPeriodoMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return res.status(500).json({
      message: "Erro interno ao verificar o período de submissão. Por favor, tente novamente mais tarde.",
      error: erro instanceof Error ? erro.message : "Erro na verificação do período de upload.",
    });
  }
};
