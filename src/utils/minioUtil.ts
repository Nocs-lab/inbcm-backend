 ...
 */
export const generateFilePath = (fileName: string, museuId: string, declarationYear: string, archiveType: string): string => {
    const now  = Date.now();
 ...
 */
export async function getLatestPathArchive(bucketName: string, prefix: string): Promise<string | null> {
  try {
    const objects = [];
...
export const uploadFileToMinio = async (
  ...
  const objectPath = generateFilePath(file.originalname, museumId, declarationYear, fileType);
